...
import { copy as copyBytes } from "../../bytes/0.223.0/copy.js";
import { assert } from "../../assert/0.223.0/assert.js";
...
export function readRangeSync(
  r: ReaderSync & dntShim.Deno.SeekerSync,
  range: ByteRange,
): Uint8Array {
  // byte ranges are inclusive, so we have to add one to the end
  let length = range.end - range.start + 1;
