...
`;
const EVENT_RESOLVE_BEGIN = "resolveBegin", EVENT_PAGE_END = "endPage";
const mutateMermaidBlockPart = (part) => {
...
export const load = (app) => {
    ...
    app.converter.on(EVENT_RESOLVE_BEGIN, (ctx) => {
    ...
    app.renderer.on(EVENT_PAGE_END, (event) => {
        const contents = event.contents;
        if (!contents?.includes(mermaid_class_str)) {
            return;
        }
        ...
        event.contents = (contents.slice(0, bodytag_end_index)
            + mermaid_script
