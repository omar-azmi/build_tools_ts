...
export async function createGraph(rootSpecifiers, options = {}) {
    rootSpecifiers = Array.isArray(rootSpecifiers)
        ? rootSpecifiers
        : [rootSpecifiers];
    const { load = defaultLoad, defaultJsxImportSource, defaultJsxImportSourceTypes, jsxImportSourceModule, cacheInfo, resolve, resolveTypes, kind, imports, } = options;
    ...
    return await createGraph(rootSpecifiers, async (specifier, options) => {
        ...
        return result;
    }, defaultJsxImportSource, defaultJsxImportSourceTypes, jsxImportSourceModule, cacheInfo, resolve, resolveTypes, kind, imports);
}
...
export function parseModule(specifier, content, options = {}) {
    const { headers, defaultJsxImportSource, defaultJsxImportSourceTypes, jsxImportSourceModule, resolve, resolveTypes, } = options;
    if (!wasm.isInstantiated()) {
        throw new Error("Please call `init()` at least once before calling `parseModule`.");
    }
    return wasm.parseModule(specifier, headers, defaultJsxImportSource, defaultJsxImportSourceTypes, jsxImportSourceModule, content, resolve, resolveTypes);
}
