...
import { colors, fromFileUrl } from "./deps.js";
...
export class FileFetcher {
    ...
    #fetchCached(specifier, redirectLimit, options, httpCache) {
        if (redirectLimit < 0) {
    ...
    async #fetchRemote(specifier, redirectLimit, options, httpCache) {
        if (redirectLimit < 0) {
            throw new dntShim.Deno.errors.Http(`Too many redirects.\n  Specifier: "${specifier.toString()}"`);
        }
        if (shouldUseCache(options.cacheSetting, specifier)) {
            const response = this.#fetchCached(specifier, redirectLimit, options, httpCache);
            if (response) {
                return response;
            }
        }
    ...
    async fetch(specifier, options) {
        ...
        if (response) {
            ...
            const response = await this.#fetchRemote(specifier, 10, this.#resolveOptions(options), await this.#resolveHttpCache());
            if (response) {
                await this.#cache.set(specifier.toString(), response);
            }
            return response;
