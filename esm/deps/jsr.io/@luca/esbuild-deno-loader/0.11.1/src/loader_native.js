...
import { dirname, fromFileUrl, join } from "../../../../@std/path/1.0.8/mod.js";
import { encodeBase32 } from "../../../../@std/encoding/1.0.7/base32.js";
import { lastIndexOfNeedle } from "../../../../@std/bytes/1.0.5/mod.js";
import * as deno from "./deno.js";
import { rootInfo } from "./deno.js";
import { mapContentType, mediaTypeFromSpecifier, mediaTypeToLoader, parseNpmSpecifier, } from "./shared.js";
...
export class NativeLoader {
    ...
    #infoCache;
    #linkDirCache = new Map(); // mapping from package id -> link dir
    constructor(options) {
    ...
    async loadEsm(specifier) {
        ...
        const entry = await this.#infoCache.get(specifier.href);
        if ("error" in entry && specifier.protocol !== "file:" &&
            mediaTypeFromSpecifier(specifier) !== "Unknown")
            throw new Error(entry.error);
        if (!("local" in entry)) {
        ...
        if (loader === null)
            return undefined;
        let contents = await dntShim.Deno.readFile(entry.local);
    ...
    async #nodeModulesDirForPackageInner(npmPackageId, npmPackage) {
        ...
        await dntShim.Deno.mkdir(tmpDirParent, { recursive: true });
        const tmpDir = await dntShim.Deno.makeTempDir({ dir: tmpDirParent });
