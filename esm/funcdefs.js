...
*/
export const createPackageJson = async (deno_json_path = default_deno_json_path, overrides = {}) => {
...
*/
export const gitRepositoryToUrl = (repo_git_url) => {
...
*/
export const gitRepositoryToPagesUrl = (repo_git_url) => {
    const repo_url = gitRepositoryToUrl(repo_git_url), [user_name, repo_name] = trimSlashes(repo_url.pathname).split("/");
...
`);
            ...
            for await (const src_dir_entry of expandGlob(src, { root: abs_deno_dir })) {
                const abs_src = src_dir_entry.path, is_file = src_dir_entry.isFile, is_folder = src_dir_entry.isDirectory, basename = src_dir_entry.name, abs_dst = is_folder
                    ? pathResolve(dir, dst)
...
export const createFiles = async (input_files, config) => {
    ...
    for (let [dst_path, content, options] of input_files) {
        ...
        if (data_is_stream) {
            const { kind, stream } = (await detectReadableStreamType(content));
