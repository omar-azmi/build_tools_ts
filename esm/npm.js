...
export const buildNpm = async (build_config = {}) => {
    const { dir, deno, copy = [], text = [], dnt, log, dryrun = false } = { ...defaultBuildNpmConfig, ...build_config }, abs_dir = pathResolve(dir), abs_deno = pathResolve(deno), log_is_verbose = log === "verbose", log_is_basic = log_is_verbose || log === "basic";
    const { name: library_name = "library", exports, } = await getDenoJson(deno);
    const mainEntrypoint = typeof exports === "string" ? exports : exports["."];
    if (log_is_verbose) {
        console.log("the main entry-point of your npm-build will be:", mainEntrypoint);
    }
    if (log_is_verbose) {
        console.log("current npm-build configuration is:", { dir, deno, copy, text, dnt, log, dryrun });
    }
    if (log_is_verbose) {
    ...
    if (log_is_verbose) {
        console.log("[in-fs] emptying your npm-build directory:", abs_dir);
    }
    if (!dryrun) {
        await emptyDir(dir);
    }
    if (log_is_basic) {
        console.log("[in-fs] transforming your deno project to an npm-build via \`dnt\`");
    }
    if (!dryrun) {
        await dntBuild({
    ...
    await copyAndCreateFiles({ dir, deno, copy, text: [...text, tsconfig_text_creation], log, dryrun });
    return {
        dir: abs_dir,
        files: [],
        cleanup: async () => {
            if (log_is_basic) {
                console.log("[in-fs] deleting your npm-build directory:", abs_dir);
            }
            if (dryrun) {
                return;
            }
